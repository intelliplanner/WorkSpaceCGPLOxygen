...
//		IntStream.range(0, arr.size()-1).filter(i -> (nth-1)-i == 0).map(m-> arr.get(m)).forEach(System.out::println);
		
		IntStream.range(0, arr.size()-1).filter(i -> (nth-1)-i == 0).mapToObj(m-> arr.toArray()[m]).forEach(System.out::println);
