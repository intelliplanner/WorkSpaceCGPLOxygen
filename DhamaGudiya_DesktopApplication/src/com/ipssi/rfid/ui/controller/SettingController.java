...
public class SettingController implements Initializable, ControllerI {
	...
	boolean gateInVisible = false;
	boolean tareVisible = false;
	boolean grossVisible = false;
	boolean gateOutVisible;
	boolean dbSettingVisible = false;
	boolean systemConfigMenuVisible = false;
	boolean rfidConfigMenuVisible = false;
	boolean wbConfigMenuDisable = false;
	boolean barrierDisable = false;
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		if (TokenManager.PROJECT_AREA == LovType.PROJECT_SIDE.DHAMA_GUDIYA) {
			...
			grossVisible = true;
			gateOutVisible = true;
			dbSettingVisible = true;
			wbConfigMenuDisable = true;
			barrierDisable = true;
			readTagDisable = true;
		}

		setConfigurationMenuDisable(gateInVisible, tareVisible, grossVisible, gateOutVisible, dbSettingVisible,
				systemConfigMenuVisible, rfidConfigMenuVisible, wbConfigMenuDisable, barrierDisable, readTagDisable);
	...
	private void systemConfigAction() {
		loadScreen(ScreenConstant.ScreenLinks.SYSTEM_CONFIG_SETTING_WINDOW,
				ScreenConstant.MenuItemId.SYSTEM_CONFIG_SETTING_WINDOW);
		setScreenComponentVisible(true);
	}

	private void wbGrossConfigAction() {
		loadScreen(ScreenConstant.ScreenLinks.WB_GROSS_SETTING_WINDOW,
				ScreenConstant.MenuItemId.WB_GROSS_SETTING_WINDOW);
		setScreenComponentVisible(true);
	}
	...
	private void readTagConfigAction() {
		loadScreen(ScreenConstant.ScreenLinks.TAG_READ_SETTING_WINDOW,
				ScreenConstant.MenuItemId.TAG_READ_SETTING_WINDOW);
		setScreenComponentVisible(true);
	}
	...
	public void setConfigurationMenuDisable(boolean gateInVisibles, boolean tareVisibles, boolean grossVisibles,
			boolean gateOutVisibles, boolean dbSettingVisibles, boolean systemConfigMenuVisibles,
			boolean rfidConfigMenuVisibles, boolean wbConfigMenuDisables, boolean barrierDisables,
			boolean readTagDisables) {
		CONTROL_GATE_IN_SETTING.setDisable(gateInVisibles);
		CONTROL_WB_TARE_SETTING.setDisable(tareVisibles);
		CONTROL_WB_GROSS_SETTING.setDisable(grossVisibles);
		CONTROL_GATE_OUT_SETTING.setDisable(gateOutVisibles);
		CONTROL_DATABASE_SETTING.setDisable(dbSettingVisibles);
		CONTROL_WEIGHBRIDGE_CONFIG_SETTING.setDisable(wbConfigMenuDisables);
		CONTROL_BARRIER_CONFIG_SETTING.setDisable(barrierDisables);
		CONTROL_SYSTEM_CONFIG_SETTING.setDisable(systemConfigMenuVisibles);
		CONTROL_RFID_CONFIG_SETTING.setDisable(rfidConfigMenuVisibles);
		CONTROL_TAG_READ_CONFIG_SETTING.setDisable(readTagDisables);
	}
