...
public class TransporterDetailWindowController implements Initializable,ControllerI {
	...
	private static final Logger log = Logger.getLogger(TransporterDetailWindowController.class.getName());
    /**
	...
	public boolean save() {
		...
		try {
			...
			if (Utils.isNull(TEXT_TRANSPORTER_NAME.getText())) {
				...
				String stdName = Utils.isNull(TEXT_TRANSPORTER_STD_NAME.getText()) ? TEXT_TRANSPORTER_NAME.getText() : TEXT_TRANSPORTER_STD_NAME.getText();
				GateInDao.insertTransporter(conn, TEXT_TRANSPORTER_NAME.getText(),stdName, TEXT_TRANSPORTER_SAP_CODE.getText());
				conn.commit();
				clearInputs();
