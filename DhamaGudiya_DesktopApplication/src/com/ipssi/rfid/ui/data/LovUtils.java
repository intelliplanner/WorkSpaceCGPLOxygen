...
public class LovUtils {
    ...
    public static void setLov(Connection conn, int portNodeId, JFXComboBox comboBox, LovDao.LovItemType lovItemType,
        ...
        switch (lovItemType) {
            ...
            case PROJECT_AREA:
                lovList = new ArrayList<ComboItem>();
                 entryType = LovType.PROJECT_SIDE.CGPL;
                lovList.add(new ComboItem(entryType, LovType.PROJECT_SIDE.getStr(entryType)));
                entryType = LovType.PROJECT_SIDE.DHAMA_GUDIYA;
                lovList.add(new ComboItem(entryType, LovType.PROJECT_SIDE.getStr(entryType)));
                itemListPair = new Pair<Integer, ArrayList<ComboItem>>(findItemIndex(lovList, selectedValue), lovList);
                break;
            case PRINTER:
            ...
            case DHAMA_GODIYA_TRANSPORTER:
                lovList = new ArrayList<ComboItem>();
                if (MainController.transporterList == null || MainController.transporterList.size() == 0) {
                    return;
                }
                for (int i = 0; i < MainController.transporterList.size(); i++) {
                    entryType = MainController.transporterList.get(i).getId();
                    lovList.add(new ComboItem(entryType, MainController.transporterList.get(i).getName()));
                }
                itemListPair = new Pair<Integer, ArrayList<ComboItem>>(findItemIndex(lovList, selectedValue), lovList);
                break;
            case DHAMA_GODIYA_MINES:
                lovList = new ArrayList<ComboItem>();
                if (MainController.minesList == null || MainController.minesList.size() == 0) {
                    return;
                }

                for (int i = 0; i < MainController.minesList.size(); i++) {
                    entryType = MainController.minesList.get(i).getId();
                    lovList.add(new ComboItem(entryType, MainController.minesList.get(i).getName()));
                }
                itemListPair = new Pair<Integer, ArrayList<ComboItem>>(findItemIndex(lovList, selectedValue), lovList);
                break;
            case DHAMA_GODIYA_DO_NO:
                lovList = new ArrayList<ComboItem>();
                if (MainController.doList == null || MainController.doList.size() == 0) {
                    return;
                }

                for (int i = 0; i < MainController.doList.size(); i++) {
                    entryType = MainController.doList.get(i).getId();
                    lovList.add(new ComboItem(entryType, MainController.doList.get(i).getName()));
                }
                itemListPair = new Pair<Integer, ArrayList<ComboItem>>(findItemIndex(lovList, selectedValue), lovList);
                break;
            case DHAMA_GODIYA_GRADE:
                lovList = new ArrayList<ComboItem>();
                if (MainController.gradeList == null || MainController.gradeList.size() == 0) {
                    return;
                }

                for (int i = 0; i < MainController.gradeList.size(); i++) {
                    entryType = MainController.gradeList.get(i).getValue();
                    lovList.add(new ComboItem(entryType, MainController.gradeList.get(i).getLabel()));
                }
                itemListPair = new Pair<Integer, ArrayList<ComboItem>>(findItemIndex(lovList, selectedValue), lovList);
                break;
