...
public class JsonStreamer {
	...
	public static JSONArray printMultiJason(Table table, StringBuilder sb,SessionManager _session, JspWriter out) throws IOException {
	...
	public static JSONArray printMultiJasonOld(Table table, StringBuilder sb,SessionManager _session, JspWriter out) throws IOException {
		JSONArray retval = new JSONArray();
		ArrayList<TD> rowData = null;
		ArrayList<TR> header = table.getHeader();
		ArrayList<TR> body = table.getBody();
		...
		for (int row = 0, rowSize = body == null ? 0 : body.size(); row < rowSize; row++) {
			...
			for (int col = 0, size = tdArr == null ? 0 : tdArr.size(); col < size; col++) {
				try {
					...
					if (hasNestedTable) {
						StringBuilder nestedSB = new StringBuilder();
						nestedJson = JasonGenerator.printJason(td.getNestedTable(), nestedSB, _session);// JsonStreamer.printJason(td.getNestedTable(), nestedSB, _session, out);
						tempColVal = nestedJson;
						colVal = nestedJson.toString();
					} else {
						colVal = td.getContent();
...
//					jsObject.append("\""+key+"\":\""+value+"\"");
	...
	public static JSONArray printJason(Table table, StringBuilder sb,SessionManager _session, JspWriter out) throws IOException {
		ArrayList<TD> rowData = null;
		ArrayList<TR> header = table.getHeader();
		ArrayList<TR> body = table.getBody();
...
//			StringBuilder jsObject = new StringBuilder();
			...
			for (int col = 0; col < colSize; col++) {
				try {
					...
					if (hasNestedTable) {
						StringBuilder nestedSB = new StringBuilder();
						nestedJson = JsonStreamer.printJason(td.getNestedTable(), nestedSB, _session, out);
						tempColVal = nestedJson;
						colVal = nestedJson.toString();
					} else {
						colVal = td.getContent();
					...
					if (noHeader && (col % 2 == 1)) {
						...
						try {
							if (headerSB != null && !"".equals(headerSB.toString()) && !hasHeader) {
								...
								out.print("["+objectHeader.toString());
								System.out.println("printJason() sb: "+ objectHeader.toString());
								hasHeader = true;
								sqBrStarted = true;
							}
						} catch (JSONException e) {
...
//						jsObject.append("\""+key+"\":\""+value+"\"");
					...
					try {
						if (headerSB != null && !"".equals(headerSB.toString())	&& !hasHeader) {
							JSONObject objectHeader = new JSONObject();
							objectHeader.putOpt("header", headerSB.toString());
//							jsObject.append("\""+"header"+"\":\""+headerSB.toString()+"\"");
							out.print("["+objectHeader.toString());
							System.out.println("printJason() sb: "+ objectHeader.toString());
							hasHeader = true;
							sqBrStarted = true;
...
//					jsObject.append("\""+key+"\":\""+value+"\"");
			...
			System.out.println("printJason() sb: "+ jsonObject.toString());
			if(sqBrStarted){
	...
	public static void main(String[] args) {
		JsonStreamer s = new JsonStreamer();
