...
public class GenerateData extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			else if ("xmlData".equals(action))
				processXMLData(request, response);
			else if ("config_file".equals(action))
				processConfigData(request, response);
			else if ("auto_email".equals(action)){
				if(isMail)
	...
	private boolean processDownload(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			if(menuBean != null){
				...
				PrintDateTable qb1 = new PrintDateTable();
		        qb1.printPage(_dbConnection, fPageInfo, _session, searchBoxHelper, buffer, reportType, reportName,reportId);
				}
				else{
					GeneralizedQueryBuilder qb = new GeneralizedQueryBuilder();
					qb.printPage(_dbConnection , fPageInfo, _session , searchBoxHelper,null,buffer, reportType, reportName,reportId,null,null,null, null, null);
				}
			}
			if(buffer.size() > 0){
				stream = response.getOutputStream();
