...
public class DataProcessorQueueSender {
	// static Logger logger = Logger.getLogger(DataProcessorQueueSender.class);
	...
	private static int maxRetryAttempt = 50;
	...
	static {
		...
		try {
			...
			try {
				...
				System.out.println("DataProcessorQueueSender.enclosing_method() Problem while intitializing the DataProcessor QueueMsgSender " + ex);
				ex.printStackTrace();
			}

		} catch (Exception ex) {
			// logger.error(" Problem while intitializing the QueueMsgSender " + ex);
			System.out.println("DataProcessorQueueSender.enclosing_method() Problem while intitializing the QueueMsgSender " + ex);
			ex.printStackTrace();
		}
	...
	public synchronized static void reInit() {
		...
		if (retryCount > maxRetryAttempt) {
			System.out.println("DataProcessorQueueSender.reInit() Max Retry Count Exceeded..will not Try Re Initalize further");
			return;
		}
		try {
			Properties props = PropertyManager.getProperties();
			...
			// logger.error(" Problem while intitializing the DataProcessor QueueMsgSender " + ex);
			System.out.println("DataProcessorQueueSender.enclosing_method() Problem while Re-intitializing the DataProcessor QueueMsgSender " + ex);
			ex.printStackTrace();
		}
	}
	...
	public static void send(String text, int vehicleId) throws JMSException, NamingException {
		//System.out.println("DataProcessorQueueSender.send()text  ######" + text);
		int maxTries = 10;
		int currTries = 0;
		...
		try {
			...
			e.printStackTrace();	
			System.out.println("DataProcessorQueueSender.send() Error while writing to queue...will try to reInit nexttime");
			sender = null;
			session = null;
		}
	}

	public static QueueSender getSender(int groupId) throws JMSException, NamingException {
		try {
//			System.out.println("DataProcessorQueueSender.getSender() Writing to " + ((groupId) % 11));
			return senderMap.get((groupId) % 11);
		} catch (Exception e) {
			e.printStackTrace();
			return sender;
		}
...
//		mdb.msgHandler(conn, "CURRENT,17061,028.652386,077.330968,2012-10-22 12:37:40,2012-10-22 12:38:10,0.0,145.33,null,00000000,0");
		...
		try {
			DataProcessorQueueSender.send("DATA,17766,021.964571,082.036780,2012-12-06 13:55:59,2012-12-06 13:57:59,1.0,33.0,11967657,1010,00", 17766);
