...
public class InvPileAction implements ActionI{
	...
	throws ServletException, IOException, Exception {
		...
		try{
			...
			ArrayList<InvPileBean> infoList = getInvPile(request);
		...
		actionForward = "/inventory_pile.jsp";
	...
	public void saveInvPile(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ArrayList<InvPileBean> beanList = new ArrayList<InvPileBean>();
	    for (Node n = xmlDoc == null || xmlDoc.getDocumentElement() == null ? null : xmlDoc.getDocumentElement().getFirstChild(); n != null; n = n.getNextSibling()) {
	    	if (n.getNodeType() != 1)
	    		continue;
	    	Element e = (Element) n;
			InvPileBean bean = new InvPileBean(Misc.getParamAsInt(e.getAttribute("id")), Misc.getParamAsInt(e.getAttribute("port_node_id"), v123), Misc.getParamAsString(e.getAttribute("short_code"),null)
					...
					, Misc.getParamAsInt(e.getAttribute("region_id")), Misc.getParamAsInt(e.getAttribute("landmark_id")), Misc.getParamAsInt(e.getAttribute("priority")), Misc.getParamAsString(e.getAttribute("landmark_name")), Misc.getParamAsString(e.getAttribute("region_name")), Misc.getParamAsInt(e.getAttribute("pit_id")), Misc.getParamAsDouble(e.getAttribute("difficulty_factor"), 1));
	    ...
	    InvPileBean.saveInvPiles(session.getConnection(), beanList);
//		TripProcessorGateway.refreshLoadPile(v123);

	}
	
	public ArrayList<InvPileBean> getInvPile(HttpServletRequest request) throws Exception {
		ArrayList<InvPileBean> retval = new ArrayList<InvPileBean>();
		SessionManager session = InitHelper.helpGetSession(request);
		Cache _cache = session.getCache();
		Connection conn = session.getConnection();
		String pgContext = Misc.getParamAsString(session.getParameter("page_context"), "tr_load_locations");
		...
		retval = InvPileBean.getInvPiles(conn,v123, status,pileType);
