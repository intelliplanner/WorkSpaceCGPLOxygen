...
public class RFIDSyncMaster1 {
	...
	public static int getVehicle(Connection conn, String epcId,
			String vehicleName) {
	...
	public static Triple<TPRecord, Integer, Boolean> getLatestTPR(
		...
		try {
			if (!Misc.isUndef(vehicleId)) {
				...
				if (tpr == null) {
					...
					tpr.setTprStatus(0);// TPR.OPEN=0
					if (holder != null && holder.isDataOnCard()
							&& holder.getValidityFlag()) {
	...
	public static StringBuilder setRFIDData(Connection conn, String dataStr,
			String epcId, String vehicleName, int deviceId, int userId,
			int updatedBy, String orgName) throws SQLException {
		StringBuilder retval = new StringBuilder();
		// PreparedStatement ps = null;
		// ResultSet rs = null;
		int logStatus = 0;
		...
		try {
			...
			if (epcId != null && epcId.length() > 0 && data != null
					&& data.length > 0) {
				tag = new RFIDTagInfo();
			...
			logStatus = insertHHLogData(conn, tag, holder, userId, updatedBy,
					epcId);
			...
			if (holder != null) {
				...
				ArrayList<Object> mines_list = RFIDMasterDao.select(conn,
						mines_details);
				String unitCode = "";
				int finYear = 0;
				if (mines_list != null) {
				...
				TPRecord tpr;
				if (true || orgName.equalsIgnoreCase("DVC")) { 
					tpr = TPRInformation.getTPRForHHWeb(conn, holder
							.getVehicleId(), holder, holder.getVehicleName());
				} else {
					tpr = TPRInformation.getTPRForHHWeb(conn, holder
							.getVehicleId(), holder, holder.getVehicleName());
				}
				if (tpr != null) {
					...
					tpr.setWbChallanNo(unitCode + finYear
							+ holder.getPreChallanId());
	...
	public static StringBuilder setRFIDDataMulti(Connection conn,
			byte[] dataByte) throws SQLException {
		StringBuilder retval = new StringBuilder();
		// PreparedStatement ps = null;
		// ResultSet rs = null;
		...
		while (++recordindex <= recordCount) {
			...
			try {
				...
				// retval.append("<result>");
				// data =
				// Utils.GetBytesFromBinaryString(dataStr);//+"0010000000100000");
				...
				System.out.println("[Web RFID EPC]:" + epcId);
				if (epcId != null || vehicleName != null)
					vehPair = TPRInformation.getVehicle(conn, epcId,
							vehicleName);
				if (vehPair != null && holder != null) {
					holder.setVehicleId(vehPair.first);
				}
				logStatus = insertHHLogData(conn, tag, holder, 1, holder
						.getUpdatedBy(), holder.getEpcId());
				System.out.println("logStatus:" + logStatus);
				if (logStatus != com.ipssi.gen.utils.Misc.getUndefInt()) {
				...
				if (holder != null) {
					System.out.println("Web rfid data recieved");
					holder.printData();
					TPRecord tpr = TPRInformation.getTPRForHHWeb(conn, holder
							.getVehicleId(), holder, holder.getVehicleName());
					if (tpr != null) {
						// if(Misc.isUndef(tpr.getTprId()))
						TPRInformation.insertUpdateTpr(conn, tpr);
	...
	public static StringBuilder getMines(Connection conn, int deviceId,
		...
		String query1 = "select mines_details.id,mines_details.name,mines_details.challan_prefix "
				...
				// " mines_devices  " +
				// " join " +
				" mines_details join mines_devices on mines_devices.mines_id=mines_details.id where  mines_details.status=1 and mines_devices.device_id="
				+ deviceId + " and mines_details.updated_on > ?";
			...
			while (rs.next()) {
				...
				retval.append("<field I=\"").append(Misc.getRsetInt(rs, "id"))
						.append("\" N=\"").append(rs.getString("name")).append(
								"\" P=\"").append(
								rs.getString("challan_prefix")).append("\" />");
			}
			if (retval != null)
				retval.append("</object>");
		} catch (Exception ex) {
	...
	public static StringBuilder getTransporters(Connection conn, int deviceId,
			int userId, long lastSync, int syncStatus) throws SQLException {
		StringBuilder retval = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		String query1 = "select transporter_details.id,transporter_details.name,transporter_details.lr_prefix "
				...
				+ " and transporter_details.updated_on > ?";
		...
		try {// mines_details
			...
			while (rs.next()) {
				...
				retval.append("<field I=\"").append(Misc.getRsetInt(rs, "id"))
						.append("\" N=\"").append(rs.getString("name")).append(
								"\" P=\"").append(rs.getString("lr_prefix"))
						.append("\" />");
			}
			if (retval != null)
				retval.append("</object>");
	...
	public static StringBuilder getDONumbers(Connection conn, int deviceId,
			int userId, long lastSync, int syncStatus) throws SQLException {
		StringBuilder retval = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public static StringBuilder getGrades(Connection conn, int deviceId,
			int userId, long lastSync, int syncStatus) throws SQLException {
		StringBuilder retval = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		String query = "select grade_details.id,grade_details.name " + " from "
				+
				// " mines_devices join  do_rr_details on (do_rr_details.mines_id=mines_devices.mines_id) join do_grade_qty on (do_rr_details.id=do_grade_qty.do_id) join "
				// +
				" grade_details where status=1";
		...
		try {// mines_details
			...
			while (rs.next()) {
				if (retval == null) {
					retval = new StringBuilder();
					retval.append("<object id=\"").append(GRADES)
							.append("\" >");
				}
				retval.append("<field I=\"").append(Misc.getRsetInt(rs, "id"))
						.append("\" N=\"").append(rs.getString("name")).append(
	...
	public static StringBuilder getUsers(Connection conn, int deviceId,
		...
		ResultSet rs = null;
		Date last_sync = Utils.getDateTime(lastSync);
		String query1 = "select distinct users.id,USERNAME,PASSWORD,NAME from users join mines_users on users.id=mines_users.user_id join mines_devices on mines_devices.mines_id=mines_users.mines_id where isactive=1  ";// and
		// users.updated_on
		// >
		// ?
	...
	public static StringBuilder getRelations(Connection conn, int deviceId,
			int userId, long lastSync, int syncStatus) throws SQLException {
		StringBuilder retval = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		Date last_sync = Utils.getDateTime(lastSync);
		String query1 = "select do_id,grade_id,transporter_id from do_grade_transporter join do_rr_details_apprvd on (do_rr_details_apprvd.id=do_grade_transporter.do_id) where do_grade_transporter.status=1 and (lapse_date >= now() or lapse_date is null)"
				+ " and do_grade_transporter.updated_on > ?";
		...
		try {// mines_details
			...
			while (rs.next()) {
				...
				retval.append("<field D=\"").append(
						Misc.getRsetInt(rs, "do_id")).append("\" G=\"").append(
						rs.getInt("grade_id")).append("\" T=\"").append(
						rs.getInt("transporter_id")).append("\" />");
			}
			if (retval != null)
				retval.append("</object>");
		} catch (Exception ex) {
	...
	public static StringBuilder getMinesDevices(Connection conn, int deviceId,
		...
		try {// mines_details
			...
			while (rs.next()) {
				...
				retval.append("<field D=\"").append(
						Misc.getRsetInt(rs, "device_id")).append("\" M=\"")
						.append(rs.getInt("mines_id")).append("\" R=\"")
	...
	public static StringBuilder getFitness(Connection conn, int deviceId,
			int userId, long lastSync, int syncStatus) throws SQLException {
		StringBuilder retval = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {// mines_details
			...
			while (rs.next()) {
				...
				retval.append("<field N=\"").append(
						Misc.getRsetString(rs, "name")).append("\" B=\"")
						.append(rs.getInt("blacklisted")).append("\" G=\"")
						.append(rs.getInt("gps_ok")).append("\" P=\"").append(
								rs.getInt("paper_ok")).append("\" />");
			}
			if (retval != null)
				retval.append("</object>");
		} catch (Exception ex) {
	...
	public static int insertHHLogData(Connection conn, RFIDTagInfo tag,
			RFIDHolder holder, int userId, int updatedBy, String epcId)
			throws Exception {
		String query = " insert into rfid_handheld_log (" +
				" record_id, device_id, vehicle_id, vehicle_name, epc_id, " +
				" mines, transporter, record_time, do_id,challan_id, " +
				" lr_id, grade, material, load_tare, load_gross, " +
				" pre_mines, pre_device_id, pre_record_id, pre_challan_id, write_status, " +
				" tag_data, record_user, updated_by, created_on, tpr_status, " +
				" port_node_id, status, wb_challan_no, destination, load_gross_time, " +
				" load_tare_time " +
				" ) values "
				+ " (?, ?, ?, ?, ?, ?, ?, ?, ?, ?," +
				   " ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " +
		...
		try {
			ps = conn.prepareStatement(query);
			Misc.setParamInt(ps, holder.getId(), index++);
			Misc.setParamInt(ps, holder.getDeviceId(), index++);
			Misc.setParamInt(ps, holder.getVehicleId(), index++);
			ps.setString(index++, holder.getVehicleName());
			ps.setString(index++, holder.getEpcId());
			Misc.setParamInt(ps, holder.getMinesId(), index++);
			Misc.setParamInt(ps, holder.getTransporterId(), index++);
			ps.setTimestamp(index++, holder.getDatetime() == null ? null: new Timestamp(holder.getDatetime().getTime()));
			Misc.setParamInt(ps, holder.getDoId(), index++);
			ps.setString(index++, holder.getChallanId());
			ps.setString(index++, holder.getLRID());
			Misc.setParamInt(ps, holder.getGrade(), index++);
			Misc.setParamInt(ps, holder.getMaterial(), index++);
			Misc.setParamDouble(ps, holder.getLoadTare(), index++);
			Misc.setParamDouble(ps, holder.getLoadGross(), index++);
			Misc.setParamInt(ps, holder.getPreMinesId(), index++);
			Misc.setParamInt(ps, holder.getPreDeviceId(), index++);
			Misc.setParamInt(ps, holder.getPreRecordId(), index++);
			ps.setString(index++, holder.getPreChallanId());
			Misc.setParamInt(ps, (holder.getValidityFlag() ? 1 : 0), index++);
			ps.setBytes(index++, tag.userData);
			// ps.setObject(21, tag.userData);
			Misc.setParamInt(ps, userId, index++);
			Misc.setParamInt(ps, updatedBy, index++);
			ps.setTimestamp(index++,holder.getCreatedOn() == null ? new Timestamp(System.currentTimeMillis()) : new Timestamp(holder.getCreatedOn().getTime()));
			Misc.setParamInt(ps, 0, index++);
			Misc.setParamInt(ps, 463, index++);
			Misc.setParamInt(ps, 1, index++);
			ps.setString(index++, holder.getWbChallanNo());

			if (true || holder.getPreChallanId().equalsIgnoreCase("DVC")) {
				Misc.setParamInt(ps, Misc.getUndefInt(), index++);
				ps.setTimestamp(index++,
						holder.getCreatedOn() == null ? new Timestamp(System
								.currentTimeMillis()) : new Timestamp(holder
								.getCreatedOn().getTime()));
				ps.setTimestamp(index++,
						holder.getDatetime() == null ? new Timestamp(System
								.currentTimeMillis()) : new Timestamp(holder
								.getDatetime().getTime()));
			}
			...
			rs.close();
			System.out
					.println("RFIDMasterDao.insertHHLogData() ID : " + retval);
			if (ps != null)
				ps.close();
		} catch (Exception e) {
			e.printStackTrace();
			if (e.getMessage().contains("Duplicate entry")) {
				...
				try {
					...
					Misc.setParamInt(ps1,holder.getMinesId() , 2);
					ps1.setBytes(3, tag.userData);
					// ps.setObject(21, tag.userData);
					Misc.setParamInt(ps1, userId, 4);
					Misc.setParamInt(ps1, updatedBy, 5);
					ps1.setTimestamp(6, new Timestamp(System.currentTimeMillis()));
					Misc.setParamInt(ps1, 2, 7);
					Misc.setParamInt(ps1, 463, 8);
					Misc.setParamInt(ps1, 1, 9);
					ps1.executeUpdate();
					retval = 1;
					...
					rs1.close();
					System.out
							.println("RFIDMasterDao.insertHHLogData.Catch() ID : "
									+ retval);
					if (ps1 != null)
						ps1.close();
				}
	...
	public static void updateHHLogTprStatus(Connection conn, RFIDHolder holder,
			int tpr_status, int tprId) throws Exception {
		...
		try {
			...
			rs.close();
			System.out
					.println("RFIDMasterDao.updateHHLogTprStatus() RecodID : "
							+ holder.getGeneratedId() + " Tpr Status :"
							+ tpr_status);
			if (ps != null)
				ps.close();
		} catch (Exception e) {
	...
	public static StringBuilder getTableStatus(Connection conn, int deviceId,
		...
		String tables[] = { "mines_details", "do_rr_details",
				"transporter_details", "grade_details", "users",
				"do_grade_transporter", "", "mines_devices" };
		Date last_sync = Utils.getDateTime(lastSync);
		...
		for (String table : tables) {
			...
			if (index == 40007) {
				...
				if (diff == -1) {
					retval.append("<field I=\"").append(index)
							.append("\" S=\"").append(2).append("\" />");
				} else {
					retval.append("<field I=\"").append(index)
							.append("\" S=\"").append(0).append("\" />");
		...
		if (retval != null)
			retval.append("<field I=\"").append("40010").append("\" S=\"")
