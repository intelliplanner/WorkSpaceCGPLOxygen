 ...
 * @author Angie
 */
public class ScreensController_old extends StackPane {
	...
	public ScreensController_old(BorderPane bp) {
		// super();
		this.bp = bp;
	}
	...
	public boolean setScreen(final String name) {
		if (screens.get(name) != null) { // screen loaded
			...
			if (bp.getChildren().size() == 3) { // if there is more than one screen
				Timeline fade = new Timeline(new KeyFrame(Duration.ZERO, new KeyValue(opacity, 1.0)),
						new KeyFrame(new Duration(1000), new EventHandler<ActionEvent>() {
							@Override
							public void handle(ActionEvent t) {
								// bp.setCenter(null); //remove the displayed screen
								bp.setCenter(screens.get(name)); // add the screen
				...
				bp.setCenter(screens.get(name)); // no one else been displayed, then just show
				Timeline fadeIn = new Timeline(new KeyFrame(Duration.ZERO, new KeyValue(opacity, 0.0)),
		 ...
		 * getChildren().add(0, screens.get(name)); //add the screen screenToRemove =
		 * getChildren().get(1); getChildren().remove(1); //remove the displayed screen
		 * }else{ getChildren().add(screens.get(name)); //no one else been displayed,
