...
//	private int lineItem = Misc.getUndefInt();
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		if(TokenManager.AUTO_COMPLETE_ON_OFF){
		...
		try {
			...
			if (Utils.isNull(TEXT_VEHICLE_NAME.getText())) {
				...
				if (isTagIssue == 0) {
					...
					if(pairVal == null) {
						boolean isTrue = GateInDao.insertDriverDetails(conn, CacheTrack.standardizeName(TEXT_DRIVER_NAME.getText()), CacheTrack.standardizeName(TEXT_DL_NUMBER.getText()));
					}
					if(isLastTprBlocked) {
						updateBlocking(conn);
					}
