...
public class MainController implements Initializable {
	...
	public void initMenuCache() {// link menu with screens
		...
		menuItem = new MenuItemInfo(ScreenConstant.MenuItemId.TPR_DETAILS_WINDOW,
				ScreenConstant.ScreenTitle.TPR_DETAILS_WINDOW, ScreenConstant.ScreenLinks.TPR_DETAILS_WINDOW,
				true, ScreenConstant.MenuItemId.MENU_PRIV.TPR_DETAILS_WINDOW,
...
//				setMenuDisable(false, false, false, false, false, false,false);
				...
				setMenuDisable(gateInDisable, tareDisable, grossDisable, gateOutDisable, settingDisable,
						transporterDisable, readTagDisable,tprDetailsDisable);

			} else if (userData.getPrivList() != null && userData.getPrivList().size() > 0) {
				mainMenuContainerVisible=true;
				gateInDisable =  gateInDisable ? true : userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.GATE_IN) ? false : true;
				tareDisable = tareDisable ? true : userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.TARE_WB)? false : true;
				grossDisable = grossDisable ? true : userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.GROSS_WB) ? false : true;
				gateOutDisable= gateOutDisable ? true: userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.GATE_OUT)? false : true;
				settingDisable= settingDisable ? true :  userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.SETTING)? false : true;
				transporterDisable= transporterDisable ? true : userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.TRANPORTER_WINDOW)? false : true;
				readTagDisable = readTagDisable ? true: userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.READ_TAG_WINDOW)? false : true;
				tprDetailsDisable = tprDetailsDisable ? true: userData.getPrivList().contains(ScreenConstant.MenuItemId.MENU_PRIV.TPR_DETAILS_WINDOW)? false : true;
				screenComponentVisible(labelScreenTitleVisible, mainMenuContainerVisible, labelBlockingReasonVisible,
						controlSignOutVisible, mainActionContainerVisible, labelHSNVisible);
				...
				setMenuDisable(true, true, true, true, true, true,true,true);

				labelBlockingReason.setText("Please Contact with Control Room for getting menu privelege");
			}
		}
		setMenuVisible(true, true, true, true, true, true,true,true);
		// setMenuVisible(false, false, false, false, false, false);
	}
...
//		setMainActionContainerControllerVisible(TokenManager.forceManual, true, true);
	}
...
//		mainMenuContainerVisible = true;
	...
	public void setMenuDisable(boolean gateInVisible, boolean tareVisible, boolean grossVisible, boolean gateOutVisible,
			boolean settingVisible, boolean transporterMenuVisible,boolean tagReadMenuVisible,boolean tprDetailsMenuDisable) {
	...
	public void setMenuVisible(boolean gateInVisible, boolean tareVisible, boolean grossVisible, boolean gateOutVisible,
			boolean settingMenuVisible, boolean transporterMenuVisible, boolean tagReadMenuVisible,boolean tprDetailsMenuVisible) {
...
//	private void setMainActionContainerControllerVisible(boolean manualControlVisible, boolean saveControlVisible,
...
//			this.showAlert(Alert.AlertType.ERROR, "Message", UIConstant.SAVE_FAILER_MESSAGE);
		}
...
//				showAlert(Alert.AlertType.CONFIRMATION, "COLOR", "RED");
					...
					LoginController.TopPane.getStyleClass().add("topPaneRed");;//setStyle("fx-background-color: #d01212");
