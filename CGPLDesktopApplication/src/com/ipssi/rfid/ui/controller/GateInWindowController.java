...
public class GateInWindowController implements Initializable, ControllerI {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		AutoCompleteTextField driverAutoCombo = new AutoCompleteTextField(this.parent, TEXT_DL_NUMBER,
				LovDao.LovItemType.DL_NUMBER);
	...
	private void handleActionControlOnChange(String controllId) {
		...
		case "COMBO_CUSTOMER":
			...
			// LovUtils.initializeComboBox(COMBO_TRANSPORTER,
			// LovDao.LovItemType.TRANSPORTER, Misc.getUndefInt(),
			// LovUtils.getIntValue(COMBO_CUSTOMER), null);
			break;
	...
	private void handleActionControl(String controlId) {
		...
		case "COMBO_CUSTOMER":
			...
			// LovDao.LovItemType.PO_SALES_ORDER,
	...
	public boolean save() {
		...
		try {
			...
			if (Utils.isNull(TEXT_VEHICLE_NAME.getText())) {
				...
				double qty = GateInDao.getRemainingQuantity(null, LovUtils.getTextValue(COMBO_PO_SALES_ORDER),
						LovUtils.getTextValue(COMBO_LINE_ITEM));
				...
				if (isTagIssue == 0) {
					...
					Pair<String, String> pairVal = GateInDao.getDriverDetails(conn,
							CacheTrack.standardizeName(TEXT_DL_NUMBER.getText()));
					if (pairVal == null) {
						boolean isTrue = GateInDao.insertDriverDetails(conn, TEXT_DRIVER_NAME.getText(),
								CacheTrack.standardizeName(TEXT_DL_NUMBER.getText()));
					}
					if (isLastTprBlocked) {
						updateBlocking(conn);
	...
	private boolean isBlockedVehicle(Connection conn) {
		...
		for (int i = 0, is = previousTprBlockEntries == null ? 0 : previousTprBlockEntries.size(); i < is; i++) {
			...
			if (bInstruction.getType() == com.ipssi.rfid.constant.Type.BlockingInstruction.BLOCK_DUETO_UNSAFE_DRIVING) {
				if (blockingMsg.length() > 0)
					blockingMsg.append(",  ");
				blockingMsg.append(com.ipssi.rfid.constant.Type.BlockingInstruction
						.getBlockingStr(com.ipssi.rfid.constant.Type.BlockingInstruction.BLOCK_DUETO_UNSAFE_DRIVING));
				isVehicleBlocked = true;
			}

			if (bInstruction
					.getType() == com.ipssi.rfid.constant.Type.BlockingInstruction.BLOCK_DUETO_DRIVER_BEHAVIOUR) {
				if (blockingMsg.length() > 0)
					blockingMsg.append(",  ");
				blockingMsg.append(com.ipssi.rfid.constant.Type.BlockingInstruction
						.getBlockingStr(com.ipssi.rfid.constant.Type.BlockingInstruction.BLOCK_DUETO_DRIVER_BEHAVIOUR));
				isVehicleBlocked = true;
			}
	...
	public void dlNoAction() {
		if (Utils.isNull(TEXT_DL_NUMBER.getText()))
			return;
		String driverName = LovDao.getText(TokenManager.portNodeId, LovDao.LovItemType.DRIVER_NAME,
				TEXT_DL_NUMBER.getText());
	...
	public void openDialogWindow() {
		FXMLLoader fxmlLoader = new FXMLLoader(
				getClass().getResource(ScreenConstant.ScreenLinks.PRINT_GATE_PASS_WINDOW));
		Parent parentDialog;
		try {
			if (Utils.isNull(TEXT_VEHICLE_NAME.getText())) {
				parent.showAlert(Alert.AlertType.ERROR, "Message", "Please enter valid 'Vehicle Number'.");
			...
			dialogController.initData(TEXT_VEHICLE_NAME.getText(), TEXT_DL_NUMBER.getText(), TEXT_DRIVER_NAME.getText(),
					LovUtils.getTextValue(COMBO_PO_SALES_ORDER), LovUtils.getTextValue(COMBO_LINE_ITEM),
					LovUtils.getTextValue(COMBO_CUSTOMER), LovUtils.getTextValue(COMBO_TRANSPORTER),
					TEXT_NOTE.getText());
