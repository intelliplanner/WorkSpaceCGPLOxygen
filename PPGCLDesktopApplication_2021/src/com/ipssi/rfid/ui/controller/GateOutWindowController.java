 ...
 * @author Vicky
 */
public class GateOutWindowController implements Initializable, ControllerI, ActionControllerI {
	@FXML
	private AnchorPane GATE_OUT_ROOT_ANCHORPANE;
	@FXML
				...
				public void manageTag(Connection conn, Token _token, TPRecord tpr, TPStep tps,
					...
					try {
						...
						parent.labelBlockingReason
								.setText("Vehicle No: " + tpr.getVehicleName() + ", Tag EPC: " + _token.getEpcId());
	...
	public boolean save() {
		...
		System.out.println("######### Start Gate Out save() ######### ");
		...
		try {
			...
			if (Utils.isNull(TEXT_VEHICLE_NAME.getText())) {
				...
				//// setBlockingStatus();
				//// tprBlockManager.setTprBlockStatus(conn,
				// tpRecord.getTprId(),TokenManager.userId);
				//
				// }
		...
		System.out.println("######### End Gate Out save() ######### ");
		return true;
	}
	...
	private void unregisteredRFIDFromVehicle(Connection conn) {
		...
		try {
			RFIDMasterDao.executeQuery(conn,
					"update vehicle set last_epc=rfid_epc where id = '" + tpRecord.getVehicleId() + "'");
			RFIDMasterDao.executeQuery(conn,
					"update vehicle set rfid_epc=null where id = '" + tpRecord.getVehicleId() + "'");
		} catch (Exception e) {
	...
	// for (int i = 0, is = tprBlockEntries == null ? 0 : tprBlockEntries.size(); i
	// < is; i++) {
	// tprBlockEntry = tprBlockEntries.get(i);
	// BlockingInstruction bInstruction =
	// tprBlockManager.getInstructionById(tprBlockEntry.getInstructionId());
	// if(bInstruction == null)
	// continue;
	//
	// if (bInstruction.getType() ==
	...
	// }
	// if (bInstruction.getType() ==
	// com.ipssi.rfid.constant.Type.BlockingInstruction.BLOCK_DUETO_UNSAFE_DRIVING)
	// {
	// CHECKBOX_SAFE_DRIVING.setSelected(true);
	// }
	// if (bInstruction.getType() ==
	// com.ipssi.rfid.constant.Type.BlockingInstruction.BLOCK_DUETO_DRIVER_BEHAVIOUR)
	// {
