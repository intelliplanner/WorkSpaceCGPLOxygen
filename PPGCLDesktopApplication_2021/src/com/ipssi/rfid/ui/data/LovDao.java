...
public class LovDao {
	...
	public static enum LovItemType {
		VEHICLE, PO_SALES_ORDER,SALES_ORDER,SALES_ORDER_ALL, TRANSPORTER, CUSTOMER, PO_LINE_ITEM, DATA_ENTRY_TYPE, DL_NUMBER,DRIVER_NAME,AUTO_COMPLETE_VEHICLE,PROCESSING_STATUS,OPEN_CLOSE,HOUR,MINUTE,INVOICE_CANCEL,INVOICE_STATUS,PRINTER
	}
...
//			query = "select id, name,sn from customer_details where status=1 and port_node_id=? and id=?";
	...
	public static Pair<Integer, ArrayList<ComboItem>> getLovItemPair(Connection conn, int portNodeId,
		...
		switch (lovItemType) {
		case TRANSPORTER:
			query = "SELECT transporter_details.id,transporter_details.name  FROM transporter_details where status=1 and port_node_id=? order by transporter_details.name";
			break;
		case PO_SALES_ORDER:
			...
			break;
		case CUSTOMER:
			query = "Select customer_details.id,customer_details.name from customer_details  where customer_details.status=1 and customer_details.port_node_id=? order by customer_details.name";
...
//			query = "Select sap_customer_sap_code,sap_customer_name from CGPL_SALES_ORDER WHERE  STATUS=1 AND cgpl_sales_order.port_node_id=?";
		...
		case SALES_ORDER:
			query = "SELECT cgpl_sales_order.id,cgpl_sales_order.sap_sales_order FROM cgpl_sales_order where cgpl_sales_order.status =1 and cgpl_sales_order.port_node_id=? ";
			query +=	"group by cgpl_sales_order.sap_sales_order  ";
		case SALES_ORDER_ALL:
			query = "SELECT cgpl_sales_order.id,concat(cgpl_sales_order.sap_sales_order,'" + seperatorDel +"',cgpl_sales_order.sap_line_item) FROM cgpl_sales_order where cgpl_sales_order.status =1 and cgpl_sales_order.port_node_id=? ";
			break;	
		default:
			break;
		}
...
//					+ " WHERE td.status=1 AND td.port_node_id="+portNodeId +" AND ct.customer_detail_id="+otherParamVal;
			break;
		case PO_SALES_ORDER:
			query = "SELECT cgpl_sales_order.id,concat(cgpl_sales_order.sap_sales_order,'" + seperatorDel +"',cgpl_sales_order.sap_line_item) FROM cgpl_sales_order where cgpl_sales_order.status=1 and cgpl_sales_order.port_node_id="+portNodeId +" and cgpl_sales_order.customer_id="+ otherParamVal + " Order by cgpl_sales_order.sap_sales_order";
//					+ " group by cgpl_sales_order.sap_sales_order";
			break;
		case CUSTOMER:
...
//			query = "Select sap_customer_sap_code,sap_customer_name from CGPL_SALES_ORDER WHERE  STATUS=1 AND cgpl_sales_order.port_node_id="+ portNodeId +" AND cgpl_sales_order.sap_sales_order="+selectedValue;
		...
		case PO_LINE_ITEM:
			if(Utils.isNull(selectedText) || selectedText.equalsIgnoreCase("select"))
			 ...
			 query = "SELECT id,sap_line_item FROM cgpl_sales_order where cgpl_sales_order.status=1 and cgpl_sales_order.port_node_id="+portNodeId +" and cgpl_sales_order.id="+selectedValue;
			break;
		default:
			break;
		}
...
//			query = "Select customer_details.name from customer_details where customer_details.status=1 and customer_details.port_node_id=? and customer_details.id=? ";
	...
	public static List<String> getSuggestion(int portNodeId, LovItemType suggestionType, String inputText) {
		...
		switch (suggestionType) {
		case VEHICLE:
			query = " select vehicle.id,vehicle.std_name from vehicle where status in (1) and customer_id in (" + portNodeId+ ") and name like '%" +inputText+ "%'" ;
			break;
		case PO_SALES_ORDER:
			query = "SELECT cgpl_sales_order.id,concat(cgpl_sales_order.sap_sales_order,'" + seperatorDel +"',cgpl_sales_order.sap_line_item) FROM cgpl_sales_order where cgpl_sales_order.status =1 and cgpl_sales_order.port_node_id in (" + portNodeId+ ") and cgpl_sales_order.sap_sales_order like '%" +inputText+ "%' order by cgpl_sales_order.sap_sales_order"  ;
			break;
		default:
			break;
		}
