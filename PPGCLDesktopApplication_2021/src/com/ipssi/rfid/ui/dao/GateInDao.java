...
//			updatePostLotData(conn, 1, "test");
			...
			double netWt= 28.92;
			double remaining_quantiy = 23760.8 - 0;
			boolean isQuantityAvailable = remaining_quantiy < netWt ? false : true;
			System.out.println(isQuantityAvailable);
//			isSalesOrderQuantityExist()
		} catch (Exception ex) {
	...
	public static Pair<String, String> getDriverDetails(Connection conn, String dlNumber) {
		...
		String query = "SELECT driver_name,driver_dl_number from driver_details where driver_dl_number like '%"
				+ dlNumber + "%'";
		try {
			ps = conn.prepareStatement(query);
			System.out.print("GateInDao getDriverDebugData Query :" + ps.toString());
	...
	public static boolean insertTransporter(Connection conn, String name, String stdName, String sapCode)
			throws Exception {
	...
	public static Pair<Boolean, Double> isSalesOrderQuantityExist(Connection conn, String sapSalesOrder,
			String sapLineItem, double netWt) throws Exception {
...
//		int quantity = Misc.getUndefInt();
	...
	public static void updateCGPLSalesOrder(Connection conn, String sapSalesOrder, String sapLineItem, double quantity)
			throws Exception {
		PreparedStatement ps = null;
		...
		if (sapSalesOrder.contains("@"))
			sapSalesOrder = sapSalesOrder.substring(0, sapSalesOrder.lastIndexOf("@"));
	...
	// public static boolean checkSapQuantityAvailability(TprReportData
	// tprReportData) {
	...
	// public static boolean checkSapQuantityAvailability(Connection conn,
	// TprReportData tprReportData) throws Exception {
	// if (tprReportData == null)
	// return false;
	//
	// boolean isExist = false;
	...
	public static double getRemainingQuantity(Connection conn, String sapSalesOrder, String sapLineItem)
		...
		System.out.println("Remaining quantity: " + remaining_quantiy);
		return remaining_quantiy;
	}

	public static Pair<String, String> getCustomerDeatils(int intValue) {
		PreparedStatement ps = null;
		ResultSet rs = null;
		Connection conn = null;
		Pair<String, String> pairVal = null;
		String query = "SELECT sap_code,address  FROM customer_details where  STATUS = 1 and port_node_id = ? and id = ?";
		try {
			conn = DBConnectionPool.getConnectionFromPoolNonWeb();
			ps = conn.prepareStatement(query);
			ps.setInt(1, TokenManager.portNodeId);
			...
			while (rs.next()) {
				if (pairVal == null)
					pairVal = new Pair<String, String>(rs.getString(1), rs.getString(2));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
	...
	public static void setDateConverter(DatePicker datePickerInstance,DateTimeFormatter dateFormatter) {
	...
	public static SalesOrderTable getSalesOrder(int intValue, int portNodeId) {
		PreparedStatement ps = null;
		ResultSet rs = null;
		Connection conn = null;
		SalesOrderTable salesOrderObj = null;
		String query = "SELECT id,sap_sales_order,sap_sales_order_creation_date,customer_id,sap_customer_sap_code,sap_customer_name"
				+ ",sap_customer_address,sap_material,sap_line_item," + 
				"sap_order_quantity,sap_order_unit,sap_order_lapse_quantity,created_on FROM cgpl_sales_order where  status = 1 and port_node_id = ? and id = ?";
		try {
			conn = DBConnectionPool.getConnectionFromPoolNonWeb();
			ps = conn.prepareStatement(query);
			ps.setInt(1, TokenManager.portNodeId);
			...
			while (rs.next()) {
				...
				salesOrderObj.setId(rs.getInt("id"));
				salesOrderObj.setSap_sales_order(rs.getString("sap_sales_order"));
				salesOrderObj.setSap_customer_name(rs.getString("sap_customer_name"));
				salesOrderObj.setSap_customer_sap_code(rs.getString("sap_customer_sap_code"));
				salesOrderObj.setSap_customer_address(rs.getString("sap_customer_address"));
				salesOrderObj.setCustomer_id(rs.getInt("customer_id"));
				salesOrderObj.setSap_sales_order_creation_date(rs.getDate("sap_sales_order_creation_date"));
				salesOrderObj.setSap_line_item(rs.getInt("sap_line_item"));
				salesOrderObj.setSap_material(rs.getString("sap_material"));
				salesOrderObj.setSap_order_quantity(rs.getString("sap_order_quantity"));
				salesOrderObj.setSapOrderLapseQuantity(rs.getString("sap_order_lapse_quantity"));
				salesOrderObj.setCreated_on(rs.getDate("created_on"));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
	...
	public static CustomerDetails getCustomerDetails(int customerId, int portNodeId) {
		PreparedStatement ps = null;
		ResultSet rs = null;
		Connection conn = null;
		...
		try {
			...
			while (rs.next()) {
				...
				customerDetailsObj.setSapCode(rs.getString("sap_code"));
				customerDetailsObj.setAddress(rs.getString("address"));
			} 
		}catch (Exception ex) {
			ex.printStackTrace();
		} finally {
