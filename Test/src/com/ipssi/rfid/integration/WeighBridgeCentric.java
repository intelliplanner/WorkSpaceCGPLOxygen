...
public class WeighBridgeCentric {
	...
	static {
		try{
			...
			comAddr = Misc.getParamAsString(prop.getProperty("COM_PORT"), "COM10");
			baudrate = Misc.getParamAsInt(prop.getProperty("COM_BAUDRATE"), 9600);
			dataBits = Misc.getParamAsInt(prop.getProperty("COM_DATABITS"), 8);
			parity = Misc.getParamAsInt(prop.getProperty("COM_PARITY"), 0);
			stopBits = Misc.getParamAsInt(prop.getProperty("COM_STOPBITS"), 1);
			changeSignalCommand = Misc.getParamAsString(prop.getProperty("COMMAND"), "G").getBytes()[0];
	...
	public static void main(String[] args) {
		WeighBridgeCentric.changeSignal();
		try {
	...
	public static void startMonitor(long millis){
    	...
    	try{
				...
				public void run() {
					while(true){
						try{
							synchronized (lock1) {
								if(connectionStatusHandler != null){
									connectionStatusHandler.setConnectionStatus(Misc.getUndefInt(), PeripheralType.CENTRIC, isConnected() ? PeripheralStatus.CONNECTED : PeripheralStatus.DISCONNECTED);
								}
							}
    		...
    		monitor.setName("Monitor Weigh Bridge Centric");
    		monitor.start();
    	}catch(Exception ex){
    		ex.printStackTrace();
    	}
