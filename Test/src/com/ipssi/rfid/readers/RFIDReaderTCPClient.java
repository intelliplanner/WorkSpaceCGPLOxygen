...
public class RFIDReaderTCPClient implements ReaderI {
	...
	public RFIDReaderTCPClient(String server, int port, int readerId, ConnectionStatusI connectionHandler) {
		this.server = server;
		this.port = port;
		this.sleepTimeWhenConnectionNotAvail = 30000; //Convert.ToInt32(MysqlDB.prop.get("threadSleepWithNoConnection", "300000"));
	...
	private Socket ConnectSocket(String server, int port) throws IOException {
		Socket s = null;
		Socket tempSocket;
	...
	public void Connect() {
		try {
			int retry = 0;
			boolean connTrue = isConnected();
			while (!connTrue) {
				if (retry < 2) {
					Thread.sleep(sleepTimeWhenConnectionNotAvail);
					connTrue = isConnected();
					retry++;
				} else {
					break;
				}
