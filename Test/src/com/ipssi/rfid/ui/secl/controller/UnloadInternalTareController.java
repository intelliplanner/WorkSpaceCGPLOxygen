 ...
 */
public class UnloadInternalTareController implements ControllerI,Initializable {
	@FXML
	private JFXTextField textBoxVehicleName;
	@FXML
	private Label textWBReading;
	@FXML
	private Label textUnloadGross;
	@FXML
	...
	@FXML
	private Label textLoadGross;
	@FXML
	private Label textLoadTare;
	@FXML
	...
	public void populateTPRData(int readerId, Connection conn, TPRecord tpRecord, TPStep tpStep, Vehicle vehicle) {
		...
		try {
			...
			textTransporter.setText(transporter);
			textMaterialGrade.setText(grade);
			textLoadTare.setText(Misc.isUndef(tpRecord.getLoadTare()) ? "" : tpRecord.getLoadTare()+"");
			textLoadGross.setText(Misc.isUndef(tpRecord.getLoadGross()) ? "" : tpRecord.getLoadGross()+"");
			textUnloadGross.setText(Misc.isUndef(tpRecord.getLoadGross()) ? "" : tpRecord.getUnloadGross()+"");
			calculateWeights();
		}catch (Exception ex){
			ex.printStackTrace();
		}
	...
	public boolean setTPRAndSaveNonTPRData(int readerId, Connection conn, TPRecord tpRecord, TPStep tpStep) throws Exception {
    	...
    	int portNodeId = parent.getMainWindow().getWorkStationDetails().getPortNodeId();
		String transporterName  = LovDao.getFieldText(portNodeId, tpRecord.getTransporterId(), LovItemType.TRANSPORTER, "").first;
    	...
    	vehicle.setUnloadTare(capturedWt);
    	vehicle.setUnloadTareTime(new Date(System.currentTimeMillis()));
    	RFIDMasterDao.update(conn, vehicle);
		tpRecord.setUnloadTare(Misc.getParamAsDouble(textWBReading.getText(),0));
		/*if(tpRecord.getEarliestUnloadWbOutEntry() == null)
			tpRecord.setEarliestUnloadWbOutEntry(tpStep.getEntryTime());
		tpRecord.setLatestUnloadWbOutExit(new Date(System.currentTimeMillis()));*/
		return true;
	...
	private void calculateWeights(){
		double loadTare = Misc.getParamAsDouble(textLoadTare.getText());
		double loadGross = Misc.getParamAsDouble(textLoadGross.getText());
		double unloadgross = Misc.getParamAsDouble(textUnloadGross.getText());
		double unloadTare = Misc.getParamAsDouble(textWBReading.getText());
