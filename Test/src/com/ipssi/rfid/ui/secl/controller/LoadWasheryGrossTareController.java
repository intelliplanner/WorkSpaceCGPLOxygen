 ...
 */
public class LoadWasheryGrossTareController implements ControllerI ,Initializable{
    ...
    @FXML
    private JFXTextField textBoxWasheryCode;
	...
	public void populateTPRData(int readerId, Connection conn, TPRecord tpRecord, TPStep tpStep, Vehicle vehicle) {
		...
		try {
			...
			if(tpRecord != null && !Utils.isNull(this.tpRecord.getWasheryCode())){
				Pair<String, String> washery = LovDao.getAutocompletePrintablePair(portNodeId,tpRecord.getWasheryCode(), LovItemType.WASHERY);
				textBoxWasheryCode.setText(washery.first);
				textWasheryName.setText(washery.second);
			}
	...
	private void handleTareWt(Connection conn,TPRecord tpRecord,Vehicle vehicle) throws Exception{
		tareInfo = TPRTareDetails.getTareInfo(conn, tpRecord, vehicle, true, Type.TPRMATERIAL.COAL_WASHERY);
		if(tareInfo != null){
			SimpleDateFormat sdf = new SimpleDateFormat(ScreenConstant.DATE_FORMAT_DDMMYYYY_HHMM);
	...
	public boolean setTPRAndSaveNonTPRData(int readerId, Connection conn, TPRecord tpRecord, TPStep tpStep) throws Exception {
		...
		tpRecord.setWasheryCode(LovDao.getAutoCompleteValue(textBoxWasheryCode));
		...
		//tpRecord.setDestinationCode(LovDao.getAutoCompleteValue(textBoxDestinationCode));
		...
		RFIDMasterDao.executeQuery(conn, "update vehicle set do_assigned='"+veh.getId()+"' where id="+veh.getId());
		if(weighmentStep == WeighmentStep.gross){
			tpRecord.setLoadGross(capturedWt);
			tpRecord.setRfLoadGross(capturedWt);
	...
	private boolean handleDoInformation(Connection conn,String doNumber, int doId) throws Exception{
		...
		if(doDetails != null){
			...
			if(isDoValid == null || isDoValid.first != UIConstant.YES){
				...
				textBoxWasheryCode.setText("");
				textWasheryName.setText("");
				textBoxDONo.setText("");
				...
				Pair<String, String> grade = LovDao.getAutocompletePrintablePair(portNodeId, doDetails.getGradeCode(), LovItemType.MATERIAL_GRADE);
...
//				textSourceName.setText(source == null ? "" : source.second);
				...
				textBoxDONo.setText(Printer.print(doDetails.getDoNumber()));
				...
				textDestinationName.setText(destination == null ? "" : Printer.print(destination.second));
				...
				textmaterialGradeName.setText(Printer.print(grade.second));
				...
				double doRemainingQty = doDetails.getQtyAlloc(parent.getMainWindow().getWorkStationDetails().getCode());//Misc.isUndef(doDetails.getQtyAlloc(parent.getMainWindow().getWorkStationDetails().getCode())) ? Misc.getUndefDouble() : doDetails.getQtyAlloc(parent.getMainWindow().getWorkStationDetails().getCode()) - ((Misc.isUndef(liftedQty) ? 0.0 : liftedQty) );//+ (Misc.isUndef(doDetails.getQtyAlreadyLifted()) ? 0.0 : doDetails.getQtyAlreadyLifted()) );
	...
	public TPRWorkstationConfig getWorkstationConfig(Connection conn, int readerId, Vehicle veh) {
		...
		if(weighmentStep == WeighmentStep.noWeight){
			TPRTareDetails tareInfo = null;
			try {
				tareInfo = TPRTareDetails.getTareInfo(conn, null, veh, true, Type.TPRMATERIAL.COAL_WASHERY);
			} catch (Exception e) {
		...
		return new TPRWorkstationConfig(weighmentStep == WeighmentStep.gross ? Type.WorkStationType.SECL_LOAD_WASHERY_GROSS : Type.WorkStationType.SECL_LOAD_WASHERY_TARE, Type.TPRMATERIAL.COAL_WASHERY);
	}
	@Override
